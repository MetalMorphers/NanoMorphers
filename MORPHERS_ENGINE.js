 ...
 * ============================================================
 *  MORPHERS_ENGINE.js
 *  Fichier 1 sur 12 — Moteur complet (code + UI + animation)
 *  À inscrire sur Bitcoin Ordinals UNE SEULE FOIS
 *  INSCRIPTION_ID_A
 *
 *  Ce fichier :
 *  1. Charge les librairies externes nécessaires
 *  2. Injecte le HTML de l'interface dans le body
 *  3. Injecte le CSS dans le head
 *  4. Lance le moteur (JS principal)
 *
 *  Dépend de window.MORPHER (initialisé par MORPHERS_DATA.js)
 * ============================================================
 */
...
// ──────────────────────────────────────────────────────────
// ÉTAPE A — Charger les librairies externes
// Chaque lib est chargée dynamiquement via createElement
// ──────────────────────────────────────────────────────────
var libs = [
  "https://arweave.net/4zl9I69C-dl8xQ485xZOkTjgaO6u0VDilSHmla80wgE",
  "https://arweave.net/fhQkVUPphB-QTl2XYkxEIpgg6YvzO2NHcsE82ZBKcw8",
  "https://cdn.jsdelivr.net/npm/gsap@3.13.0/dist/gsap.min.js",
  "https://cdn.jsdelivr.net/npm/html-to-image@1.11.11/dist/html-to-image.min.js",
  "https://cdn.jsdelivr.net/npm/particles.js@latest/particles.min.js",
];
...
// ──────────────────────────────────────────────────────────
// ÉTAPE B — Injecter le CSS dans le <head>
// ──────────────────────────────────────────────────────────
...
:root {
  ...
  /* ⚙️ --dna-color et --dna-rgb sont injectés dynamiquement par INIT MORPHER */
  /* Valeurs neutres par défaut — remplacées au chargement */
  --dna-color: #444444;
  --dna-rgb: 68 68 68;
...
.beamcard.active .beam {
  ...
  -webkit-animation: firebeam 0.8s ease-in forwards;
          ...
          filter: drop-shadow(0 0 2rem var(--dna-color));           
}
...
scrollbar-width: thin;
  ...
  @keyframes auraPulse {
    0%, 100% { filter: drop-shadow(0 0 3vmin var(--dna-color)); }
    50%       { filter: drop-shadow(0 0 1vmin var(--dna-color)); }
  }
  ...
  #typewriter-line2 {
    ...
    letter-spacing: 0.2em;
    text-shadow: 0 0 8px var(--dna-color);
    min-height: 1.6em;
    line-height: 1.8;
  }
  .tw-cursor {
...
// ──────────────────────────────────────────────────────────
// ÉTAPE C — Injecter le HTML dans le <body>
// ──────────────────────────────────────────────────────────
...
<!-- ⚙️ --dna-color injecté par JS via window.MORPHER.dna -->
<div class="card card-phase1" id="card-phase1" style="--gray-mode: 1;">
...
`;
// Insérer le contenu HTML dans le body
while (bodyDiv.firstChild) {
  document.body.appendChild(bodyDiv.firstChild);
}

// ──────────────────────────────────────────────────────────
// ÉTAPE D — Lancer le moteur après chargement des libs
// ──────────────────────────────────────────────────────────
loadScripts(libs, function() {

// ══════════════════════════════════════════════════════════
// MOTEUR PRINCIPAL — JS de l'animation
// ══════════════════════════════════════════════════════════
/***************************************************
...
(function() {
  ...
  // 6. ⚙️ Injecter --dna-color et --dna-rgb sur :root (document.documentElement)
  //    Comme ça TOUS les éléments lisent la bonne couleur :
  //    panel, scrollbar, circle, logo pulse, border-wrap, glyphs chrome...
  document.documentElement.style.setProperty('--dna-color', data.dna);
  document.documentElement.style.setProperty('--dna-rgb', data.dnaRgb);

  // 7. Injecter aussi sur le global-container Phase 2 (surcharge locale)
  ...
  // 8. Injecter --dna-color sur la carte Phase 1
  var cardPhase1 = document.getElementById('card-phase1');
  if (cardPhase1) {
    cardPhase1.style.setProperty('--dna-color', data.dna);
  }

  // 9. Injecter l'ID du Morpher dans le label switch
